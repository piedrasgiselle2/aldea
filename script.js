...
// Cambiá este PIN antes de publicar si querés otro.
// IMPORTANTE: en una web estática de GitHub Pages no es seguridad fuerte;
// sirve para evitar acceso accidental al panel docente.
...
function loadProgress(){
  try{
    const saved = JSON.parse(localStorage.getItem("numeria_progress"));
    if(!saved) return structuredClone(defaultProgress);
    return {
      ...structuredClone(defaultProgress),
      ...saved,
      unlocked:{...defaultProgress.unlocked,...(saved.unlocked||{})},
      completed:{...defaultProgress.completed,...(saved.completed||{})},
...
const challenges = [
  {title:"Desafío 1 · El horno de pan",icon:"🍞",text:"Para alimentar a 4 habitantes durante un día se necesitan 6 hogazas de pan. Si llegan 8 habitantes, ¿cuántas hogazas se necesitarán?",label:"Cantidad de hogazas",expected:"12"},
  {title:"Desafío 2 · Nuevos habitantes",icon:"🥖",text:"Para alimentar a 4 habitantes durante un día se necesitan 6 hogazas de pan. Si llegan 10 habitantes, ¿cuántas hogazas se necesitarán?",label:"Cantidad de hogazas",expected:"15"},
  {title:"Desafío 3 · El puesto del mercado",icon:"🪙",text:"Una capa cuesta 600 monedas y el comerciante anuncia un descuento del 50 %. ¿Cuántas monedas se pagan finalmente?",label:"Precio final en monedas",expected:"300"},
  {title:"Desafío 4 · La tela del sastre",icon:"🧵",text:"Una pieza de tela cuesta 800 monedas. Durante la feria tiene un descuento del 25 %. ¿Cuántas monedas se descuentan?",label:"Cantidad descontada",expected:"200"}
];
...
function saveProgress(){
  localStorage.setItem("numeria_progress", JSON.stringify(progress));
}
function saveState(){
...
function avatarClass(role, app, extra=""){
  return `rpg-avatar ${extra} ${roleData[role].className} skin-${app.skin} hair-${app.hair} haircolor-${app.haircolor} outfitcolor-${app.outfitcolor}`.trim();
}
function updatePreview(){
  preview.className = avatarClass(selectedCharacter, appearance);
...
document.querySelectorAll("[data-setting]").forEach(group=>{
  group.querySelectorAll("button").forEach(btn=>{
    btn.addEventListener("click",()=>{
      ...
      appearance[setting] = setting==="hair" ? btn.dataset.value : Number(btn.dataset.value);
      updatePreview();
    });
  });
});
...
function renderAvatarIn(targetId,extraClass,heldItemId){
  ...
  el.className=avatarClass(state.character,state.appearance,extraClass);
  document.getElementById(heldItemId).textContent=roleData[state.character].item;
}

function renderMap(){
  document.querySelectorAll(".map-location[data-place]").forEach(el=>{
    const place=el.dataset.place;
    el.classList.remove("available","completed");
    if(progress.completed[place]){
      el.classList.remove("locked");
      el.classList.add("completed");
      el.querySelector("small").textContent="Completado ✓";
    }else if(progress.unlocked[place]){
      el.classList.remove("locked");
      el.classList.add("available");
      el.querySelector("small").textContent="Disponible";
      if(!el.querySelector(".map-badge")){
      ...
      el.querySelector("small").textContent="Bloqueado";
      const badge=el.querySelector(".map-badge");
...
finishMission.addEventListener("click",()=>{
  ...
  progress.councilCompleted=true;
  if(!progress.rewards.includes(state.reward)) progress.rewards.push(state.reward);
  saveState();
  saveProgress();
...
/* Desbloqueo con código compartido por la docente */
document.getElementById("applyUnlockCode").addEventListener("click",()=>{
  const value=document.getElementById("studentUnlockCode").value.trim().toUpperCase();
  const message=document.getElementById("unlockMessage");
  const match=Object.entries(unlockCodes).find(([,code])=>code===value);
  if(!match){
    message.textContent="Ese código no corresponde a una apertura de Numeria.";
    return;
  }
  const [place]=match;
  progress.unlocked[place]=true;
  saveProgress();
  renderMap();
  message.textContent=`✨ ${placeNames[place]} quedó desbloqueado/a.`;
  document.getElementById("studentUnlockCode").value="";
});
...
document.getElementById("openTeacherMode").addEventListener("click",()=>{
  teacherModal.classList.remove("hidden");
  teacherModal.setAttribute("aria-hidden","false");
});
document.getElementById("closeTeacherMode").addEventListener("click",()=>{
...
document.getElementById("teacherLoginButton").addEventListener("click",()=>{
  ...
  const msg=document.getElementById("teacherLoginMessage");
  if(pin===TEACHER_PIN){
...
document.querySelectorAll(".show-code").forEach(btn=>{
  btn.addEventListener("click",()=>{
    ...
    document.getElementById("teacherCodeValue").textContent=unlockCodes[place];
    document.getElementById("teacherCodePlace").textContent=`Habilita: ${placeNames[place]}`;
    document.getElementById("teacherCodeCard").classList.remove("hidden");
  });
});

document.getElementById("downloadData").addEventListener("click",()=>{
  const rows=[["nombre_aventurero","personaje","piel","largo_pelo","color_pelo","color_vestimenta","recompensa","desafio","titulo","respuesta","comparacion_referencia","fecha_hora"]];
  state.answers.forEach(a=>{
    rows.push([state.playerName,state.character,state.appearance.skin,state.appearance.hair,state.appearance.haircolor,state.appearance.outfitcolor,state.reward,a.challenge,a.title,a.answer,a.correct_reference,a.timestamp]);
  });
...
document.getElementById("restartGame").addEventListener("click",()=>{
  if(confirm("¿Reiniciar todo el progreso local de Numeria en este dispositivo?")){
    localStorage.removeItem("numeria_clase1");
    localStorage.removeItem("numeria_player");
    localStorage.removeItem("numeria_progress");
    location.reload();
  }
});
